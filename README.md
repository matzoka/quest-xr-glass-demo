...
宇宙空間に浮かぶワイヤーフレームの部屋を舞台に、地球をコントローラーやキーボードで弾きながら、月、惑星、太陽フレア、Apollo 11風ミッション、USS Enterprise風のワープ演出、小さな隕石、人工衛星、旅客機、彗星、夜景、オーロラ、稲光などを眺められます。
...
- 太陽の縁付近から、数秒だけプロミネンス風のプラズマアークとフレア光が立ち上がります。
...
- 遠方の宇宙背景を、長い尾を引く彗星がまれに横切ります。
- 地球へ向かわず、隕石とは別の背景演出として自然に通過します。
...
- レア演出の周回中だけ `assets/star-trek-viewer.mp3` を流し、周回軌道に入った瞬間に開始して、周回を離れる瞬間に停止します。

### クリンゴン Negh'Var級

- `assets/NeghVarclass/` のOBJ / MTLモデルを読み込んで表示します。
...
- 彗星は地球へ向かう軌道ではなく、部屋のさらに奥を横切るワールド空間の背景演出として生成しています。
...
- クリンゴン Negh'Var級モデルは初回登場直前まで遅延ロードし、フェード中だけ材質の透明度を操作します。Enterpriseと同時に出ないよう、双方の出現スケジュールで排他制御しています。
- Apollo風ミッションは見やすさ優先の圧縮スケールで、打ち上げから月面着陸までをループ再生します。

## Assets and Rights

このデモにはNASA由来・three.jsサンプル由来の惑星テクスチャ、フリーのEnterprise風3Dモデル、Negh'Var級モデル、ローカル音声ファイルを含みます。再配布や公開利用の際は、各素材のライセンスと権利関係を確認してください。

`assets/enterprise_theme.mp3`、`assets/star-trek-viewer.mp3`、`assets/warp.mp3` を差し替える場合も、利用する音源の権利確認は利用者側で行ってください。
